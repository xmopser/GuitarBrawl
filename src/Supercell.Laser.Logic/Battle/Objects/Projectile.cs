namespace Supercell.Laser.Logic.Battle.Objects
{
    ...
    public class Projectile : GameObject
    {
        ...
        public override void Tick()
        {
            ...
            if (!ProjectileData.Indirect)
            {
                if (!tile.IsDestructed() && tile.Data.BlocksProjectiles && !(tile.Data.IsDestructibleNormalWeapon || (tile.Data.IsDestructible && ProjectileData.PiercesEnvironment)))
                {
                    ShouldDestructImmediately = true;
                }
                ...
                else if (tile.Data.IsDestructible && ProjectileData.PiercesEnvironment)
                {
                    tile.Destruct();
                }
            }
        ...
        private void HandleCollisions()
        {
            foreach (GameObject gameObject in GameObjectManager.GetGameObjects())
            {
                ...
                if (teamIndex == this.GetIndex() / 16 && ProjectileData.HealOwnPercent == 0) continue;
                if (teamIndex != this.GetIndex() / 16 && ProjectileData.HealOwnPercent != 0) continue;
                ...
                if (Position.GetDistance(gameObject.GetPosition()) <= radius1 + radius2)
                {
                    ...
                    if (ProjectileData.Name == "RocketGirlProjectile") continue;
                    if (ProjectileData.HealOwnPercent == 0) character.CauseDamage((Character)Source, Damage);
                    else character.CauseDamage((Character)Source, -Damage);
